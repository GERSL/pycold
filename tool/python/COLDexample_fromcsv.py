...
################################################################################
#     running COLD for a pixel-based Landsat time series provided by a csv     #
################################################################################
...
cold_result = pycold.pycold(data['dates'].to_numpy(np.int32), data[Landsat_bandname[0]].to_numpy(np.short),
                            data[Landsat_bandname[1]].to_numpy(np.short),
                            data[Landsat_bandname[2]].to_numpy(np.short),
                            data[Landsat_bandname[3]].to_numpy(dtype=np.short),
                            data[Landsat_bandname[4]].to_numpy(dtype=np.short),
                            data[Landsat_bandname[5]].to_numpy(dtype=np.short),
                            data[Landsat_bandname[6]].to_numpy(dtype=np.short), data['qa'].to_numpy(dtype=np.short))
...
# C api used MATLAB ordinal dates for easier verification, so the outputted dates have 366 days offset with
...
plot_cold = pd.DataFrame(columns=['Trend', 'Annual', 'Semiannual', 'Trimodel', 'predicted'])  # initialize a dataframe
                                                                      # for saving predicted values for each component
...
# plot different categories of breaks, natural recovery used red color, disturbance/afforestation use black (real disturbances)
