...
def test_step1_4():
    pyclassifier = PyClassifierHPC(test_config, record_path='tests/resources', thematic_path=testing_folder,
    ...
    if os.path.exists(pyclassifier.thematic_path):
        shutil.rmtree(pyclassifier.thematic_path)
    ...
    for iblock in range(pyclassifier.config['n_blocks']):
        ...
        for year in range(year_lowbound, year_uppbound + 1):
            assert os.path.exists(os.path.join(pyclassifier.thematic_path,
...
def test_train_rf_model():
    pyclassifier = PyClassifierHPC(test_config, record_path='tests/resources', thematic_path='tests/resources/feature_maps',
                                     year_lowbound=year_lowbound,
                                     year_uppbound=year_uppbound,
                                     labelmap_path=ref_path)
