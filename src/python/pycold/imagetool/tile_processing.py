...
from pycold.utils import assemble_cmmaps, get_rowcol_intile, get_doy, unindex_sccdpack, class_from_dict
...
def tileprocessing_report(
        result_log_path, stack_path, version, algorithm, dataset_info, startpoint, cold_timepoint,
        tz, n_cores, probability_threshold, conse, starting_date=0, n_cm_maps=0, year_lowbound=0,
        year_uppbound=0):
    """
...
def is_finished_assemble_cmmaps(cmmap_path, n_cm, starting_date, cm_interval):
    ...
    for count in range(n_cm):
        ...
        if not os.path.exists(
            join(
                cmmap_path, 'CM_maps_{}_{}{}.npy'.format(
                    str(ordinal_date),
        ...
        if not os.path.exists(
            join(
                cmmap_path, 'CM_date_maps_{}_{}{}.npy'.format(
                    str(ordinal_date),
                    pd.Timestamp.fromordinal(ordinal_date).year,
                    get_doy(ordinal_date)))):
            return False
...
def get_stack_date(
        dataset_info: DatasetInfo, block_x: int, block_y: int, stack_path: str,
        low_datebound: int = 0, high_datebound: int = 0, nband: int = 8):
    """
    :param dataset_info:
    ...
    dataset_info.block_height = int(
        dataset_info.n_rows / dataset_info.n_block_y)  # height of a block
    ...
    img_tstack = np.dstack([np.load(join(block_folder, f)).reshape(
...
@click.option(
    '--method', type=click.Choice(['COLD', 'OBCOLD', 'SCCDOFFLINE']),
    help='COLD, OBCOLD, SCCD-OFFLINE')
@click.option('--low_datebound', type=str, default=None,
              help='low date bound of image selection for processing. '
              'Example - 2015-01-01')
@click.option('--upper_datebound', type=str, default=None,
              help='upper date bound of image selection for processing.'
              'Example - 2021-12-31')
@click.option('--b_c2', is_flag=True, show_default=True, default=False,
              help='indicate if it is c2 or hls; if so, '
              'thermal bands will not be considered for observation selection ')
def main(
        rank, n_cores, stack_path, result_path, yaml_path, method, seedmap_path, low_datebound,
        upper_datebound, b_c2):
    # def main():
    #     rank = 29
    #     n_cores = 200
    ...
    if (dataset_info.n_cols % dataset_info.block_width != 0) or (dataset_info.n_rows % dataset_info.block_height != 0):
        print(
    ...
    for i in range(nblock_eachcore):
        ...
        if os.path.exists(join(result_path, 'COLD_block{}_finished.txt'.format(block_id))):
            print("Per-pixel COLD processing is finished for block_x{}_y{} ({})".format(block_x,
                  block_y, datetime.now(TZ).strftime('%Y-%m-%d %H:%M:%S')))
            continue
        ...
        if img_tstack is None:   # empty block
            if method == 'OBCOLD':
                ...
                np.save(join(result_path, 'CM_date_x{}_y{}.npy'.format(
                    block_x, block_y)), np.hstack(date_collect))
                np.save(join(result_path, 'CM_x{}_y{}.npy'.format(
                    block_x, block_y)), np.hstack(CM_collect))
        else:
            ...
            if method == "SCCDOFFLINE":
                ...
                f = open(
                    join(
                        result_path, 'record_change_x{}_y{}_sccd.npy'.format(
                            block_x, block_y)),
                    "wb+")
                ...
                for pos in range(dataset_info.block_width * dataset_info.block_height):
                    ...
                    try:
                        sccd_result = sccd_detect(
                            img_dates_sorted, img_tstack[pos, 0, :].astype(np.int64),
                            ...
                            t_cg=threshold, conse=conse,
                            b_c2=b_c2, pos=dataset_info.n_cols * (original_row - 1) + original_col)
                    except RuntimeError:
                        print("S-CCD fails at original_row {}, original_col {} ({})".format(original_row,
                              original_col, datetime.now(TZ).strftime('%Y-%m-%d %H:%M:%S')))
                    else:
                    ...
                    try:
                        if method == 'OBCOLD':
                            [
                                ...
                                conse=conse,
                                starting_date=starting_date, n_cm=n_cm_maps, b_c2=b_c2,
                            ...
                            cold_result = cold_detect(
                                ...
                                img_tstack[pos, 6, :].astype(np.int64),
                                img_tstack[pos, 7, :].astype(np.int64),
                                t_cg=threshold, conse=conse, b_c2=b_c2, pos=dataset_info.n_cols *
                                (original_row - 1) + original_col)
                    except RuntimeError:
                        print("COLD fails at original_row {}, original_col {} ({})".format(
                            original_row, original_col, datetime.now.strftime('%Y-%m-%d %H:%M:%S')))
                    except Exception:
                        if method == 'OBCOLD':
                ...
                if len(result_collect) > 0:
                    np.save(
                        join(
                            result_path, 'record_change_x{}_y{}_cold.npy'.format(
                                block_x, block_y)),
    ...
    if method == 'OBCOLD':
        if seedmap_path is None:
            ob_analyst = ObjectAnalystHPC(
                dataset_info, starting_date=starting_date, stack_path=stack_path,
                result_path=result_path)
        else:
            pyclassifier = PyClassifierHPC(
                dataset_info, record_path=result_path,
        ...
        if not is_finished_assemble_cmmaps(join(result_path, 'cm_maps'), n_cm_maps,
            ...
            if rank == 1:
                assemble_cmmaps(dataset_info, result_path, join(result_path, 'cm_maps'),
                                starting_date, n_cm_maps, 'CM', cm_output_interval, clean=False)
            elif rank == 2:
                assemble_cmmaps(dataset_info, result_path, join(result_path, 'cm_maps'),
                                starting_date, n_cm_maps, 'CM_date', cm_output_interval, clean=False)
        ...
        if seedmap_path is not None:  # we used thematic info
            if not pyclassifier.is_finished_step4_assemble():
                if rank == 1:
                    print("Starts predicting features: {}".format(
                        datetime.now(TZ).strftime('%Y-%m-%d %H:%M:%S')))
                for i in range(nblock_eachcore):
            ...
            while not pyclassifier.is_finished_step4_assemble():
                time.sleep(15)
            if rank == 1:
                print("Assemble classification map ends: {}".format(
                    datetime.now(TZ).strftime('%Y-%m-%d %H:%M:%S')))
        #########################################################################
        #                      object-based image analysis                      #
        #########################################################################
        if not ob_analyst.is_finished_object_analysis(np.arange(
    ...
    if rank == 1:
        ...
        if method == 'OBCOLD':
            tileprocessing_report(join(result_path, 'tile_processing_report.log'),
                                  stack_path, pycold.__version__, method, dataset_info, start_time,
                                  cold_timepoint, TZ, n_cores, probability_threshold, conse,
                                  starting_date, n_cm_maps, year_lowbound, year_uppbound)
        else:
            tileprocessing_report(
                join(result_path, 'tile_processing_report.log'),
                stack_path, pycold.__version__, method, dataset_info, start_time, cold_timepoint,
                TZ, n_cores, probability_threshold, conse)
        print("The whole procedure finished: {}".format(
            datetime.now(TZ).strftime('%Y-%m-%d %H:%M:%S')))
